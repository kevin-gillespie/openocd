...
LOG_DEBUG("BKPT_HARD");
    if(debugger_breakpoint_count > 0) {
        for(i = 0; i < debugger_breakpoint_count; i++) {
          retval = du_core->rvmax_jtag_read_cpu(&rvmax->jtag, DEBUGGER_HWBREAKPOINT_CONTROL_OFFSET+i*8, 1, &reg_value);
LOG_DEBUG("testing debug reg %d: %x",i,reg_value);
          if(retval != ERROR_OK)
            return retval;
...
// TODO: add check for duplicate breakpoints... investigate if openocd/gdb do this for me
          ...
          retval = du_core->rvmax_jtag_write_cpu(&rvmax->jtag, DEBUGGER_HWBREAKPOINT_DATA_OFFSET+i*8, 1, &reg_value);

          if(retval != ERROR_OK)
            return retval;

          reg_value |= DBG_HWBREAKPOINT_ENABLED;
          retval = du_core->rvmax_jtag_write_cpu(&rvmax->jtag, DEBUGGER_HWBREAKPOINT_CONTROL_OFFSET+i*8, 1, &reg_value);
...
//LOG_DEBUG("testing debug reg %d: %x",i,reg_value);
      ...
      if(reg_value & DBG_HWBREAKPOINT_ENABLED) {// breakpoint in use
        retval = du_core->rvmax_jtag_read_cpu(&rvmax->jtag, DEBUGGER_HWBREAKPOINT_DATA_OFFSET+i*8, 1, &reg_value);
...
COMMAND_HANDLER(rvmax_version_command_handler)
{
  ...
  LOG_INFO("Maxim Integrated, RVMax driver version: %d.%03d%s", RVMAX_VERSION_MAX, RVMAX_VERSION_MIN,rvmax_driver_info);
  return ERROR_OK;
}
