...
static int rvmax_auth_data_init = 0;
uint32_t rvmax_auth_data[AUTH_LEN];
static struct rvmax_du rvmax_du_adv;

static int print_crc = 0;
static int break_enabled = 0;

static int adbg_wb_burst_write(struct rvmax_jtag *jtag_info, const uint8_t *data, int size, int count, unsigned long start_address);
...
//LOG_DEBUG("+");
  ...
  if(!rvmax_auth_data_init) {
    ...
    for(i = 0; i < AUTH_LEN; i++) {
      retval = adbg_wb_burst_write(jtag_info, (const uint8_t *)&rvmax_auth_data[i], 4, 1, DEBUGGER_AUTH_OFFSET);
            ...
            int count, uint32_t start_address, uint8_t *data)
{
  ...
  if((start_address & DEBUGGER_OFFSET) != DEBUGGER_OFFSET) {
    LOG_DEBUG("Doing burst read (%d), word size %d, word count %d, start address 0x%08" PRIx32,jtag_info->rvmax_jtag_module_selected, size, count, start_address);
  } else {
...
static int adbg_wb_burst_write(struct rvmax_jtag *jtag_info, const uint8_t *data, int size, int count, unsigned long start_address)
{
  ...
  case DC_WISHBONE:
    if (size == 1)
      ...
      //LOG_DEBUG("Tried WB burst write with invalid word size (%d),"  "defaulting to 4-byte words", size);
      opcode = DBG_WB_CMD_BWRITE32;
    }
    break;
  case DC_CPU0:
    if (size == 4)
      ...
      //LOG_DEBUG("Tried CPU0 burst write with invalid word size (%d),"  "defaulting to 4-byte words", size);
      opcode = DBG_CPU0_CMD_BWRITE32;
    }
    break;
  case DC_CPU1:
    if (size == 4)
      ...
      //LOG_DEBUG("Tried CPU1 burst write with invalid word size (%d),"  "defaulting to 4-byte words", size);
      opcode = DBG_CPU0_CMD_BWRITE32;
    }
    break;
  default:
...
retry_full_write:
  ...
  if (!value) {
    ...
    LOG_ERROR("CRC ERROR!!!!! match bit after write is %" PRIi8 " (computed CRC 0x%08" PRIx32 ")", value, crc_calc);
...
static int rvmax_adv_cpu_stall(struct rvmax_jtag *jtag_info, int action)
{
  ...
  if (action == CPU_STALL) {
    ...
    if(!break_enabled) {
      ...
      adbg_wb_burst_write(jtag_info,(const uint8_t *)&value, 4, 1, DEBUGGER_BREAK_OFFSET);
      adbg_wb_burst_read(jtag_info, 4, 1, DEBUGGER_BREAK_OFFSET,(uint8_t *)&value);
    }
           ...
           uint32_t addr, uint32_t size, int count, const uint8_t *buffer)
{
  ...
  LOG_DEBUG("Writing WB%" PRId32 " at 0x%08" PRIx32, blocks_this_round, block_count_address);
      return retval;
    }
