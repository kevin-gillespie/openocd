...
CMSIS_DAP_ID=0409000080fe82cc00000000000000000000000097969906
HEXGEN=/home/btm-ci/Tools/hexgen.pl
...
-c "cmsis_dap_serial  ${CMSIS_DAP_ID}" -c "init; reset halt; max32xxx mass_erase 0; flash erase_check 0; exit"
./src/openocd -f tcl/interface/cmsis-dap.cfg -f tcl/target/${TARGET_CFG} -s tcl  \
-c "cmsis_dap_serial  ${CMSIS_DAP_ID}" -c "init; reset halt; max32xxx mass_erase 1; flash erase_check 1; exit"
./src/openocd -f tcl/interface/cmsis-dap.cfg -f tcl/target/${TARGET_CFG} -s tcl  \
-c "cmsis_dap_serial  ${CMSIS_DAP_ID}" -c "init; reset halt; max32xxx_qspi mass_erase 2; flash erase_check 2; exit"
...
./src/openocd -f tcl/interface/cmsis-dap.cfg -f tcl/target/${TARGET_CFG} -s tcl  \
    -c "cmsis_dap_serial  ${CMSIS_DAP_ID}" -c "init; reset halt; program ${TARGET}_rand.hex verify exit "
